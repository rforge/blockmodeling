...
testORPtimeLim<-function(M, #a square matrix
                         k,#number of clusters/groups
						 blockFun=kmBlockC, 
						 timeLim = 3*60, #number of seconds
                         save.initial.param=TRUE,  #save the initial parametrs of this call
                         deleteMs=TRUE, #delete networks/matrices from results of optParC or optParMultiC to save space
                         max.iden=10, #the maximum number of results that should be saved (in case there are more than max.iden results with minimal error, only the first max.iden will be saved)
                         return.all=FALSE,#if 'FALSE', solution for only the best (one or more) partition/s is/are returned
   ...
   on.exit({
     if(length(res)==0){
       res<-c(list(M=M, nStarts=i, elTime=elTime))
       class(res)<-"opt.more.par"
       return(res)
     }
     ...
     res<-c(list(M=M, nStarts=i, elTime=elTime),res,best,err,call,initial.param=initial.param, list(Random.seed=.Random.seed))
     class(res)<-"opt.more.par"
     return(res)
   })
	...
	t1<-proc.time()[3]
	i<-0
	while(elTime<timeLim){
	   i<-i+1
	   temppar<-parGenFun(n=n,k=k,mingr=mingr,maxgr=maxgr,addParam=addParam)
	   
	   res[[i]]<-blockFun(M=M, clu=temppar,  ...)
	   if(deleteMs){
		 res[[i]]$M<-NULL
	   }
	   res[[i]]$best<-NULL
	   ...
	   err[i]<-res[[i]]$err
	   elTimeOld<-elTime
	   elTime<-proc.time()[3]-t1
	 }
	 if(elTime >timeLim){
		p<-(elTime-timeLim)/(elTime-elTimeOld)
		if(runif(1)<p){
			res[[i]]<-NULL
			err<-err[-i]
			i<-i-1
