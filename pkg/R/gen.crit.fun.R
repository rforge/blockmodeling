...
#	s="default",	#suficient value for colum and row statistics
	...
	useC=FALSE, #should C subrutines be used - for testing purpuses only
  ...
){
...
#		} else homFun<-""
		...
		critFunC(M=M,clu=clu,approaches='hom', homFun='ss',blocks='com',diag=1)}"),collapse="")))
		if(changeT){
	...
	if(normal && nr==1 && all(approach=="ss") && all(blocks=="com")&& all(BLOCKS=="com") && nmode==1 && use.for){
		assign(x="useM",value=M,envir=parent.frame())
		fun1<-(parse(text=paste(c(tmpfun,"<-function(M,clu,...){
		n<-dim(M)[1]
		dn<-dimnames(M)
